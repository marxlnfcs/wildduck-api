...
export const CLIENT = createWildduckClient({
    ...
    onResponse: (data) => {
        console.log(`${data.request.method} ${data.url}: ${data.response.status} - ${data.response.statusText}`);
    },
    onError: (data) => {
        console.log(`${data.request.method} ${data.url}: Error - ${data.error.message}`);
    },
    onSSECreate: (data) => {
